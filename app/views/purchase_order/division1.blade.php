			...
			<tr>
				...
				<th>
					<div class="btn-group div-buttons">
			     	...
			     	<a class="btn btn-info btn-darkblue" href="{{$url_po_partial. '&receiver_no=' . $po->receiver_no.'&division='.$po->division. '&division_id='.$po->dept_number.'&filter_shipment_reference_no='.$filter_shipment_reference_no.'&filter_po_no='.$filter_po_no.'&total_qty='.$total_qty}}">Sync To Mobile </a>
			</tr>
		</table>
	</div>
</div>
			...
			<table class="table table-striped table-bordered">
				...
				@else
					...
					<tr class="font-size-13 tblrow" data-id="{{ $po->dept_number }}">
						...
						@endif
						<td>{{ $counter++ }}</td>
						<td><a href="{{ $url_po_detail . '&receiver_no=' . $po->receiver_no.'&division='.$po->division. '&division_id='.$po->dept_number.'&filter_stock_piler='.$po->fullname}}">{{ $po->division }}</a></td>
