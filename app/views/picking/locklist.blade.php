...
<div class="control-group">
	...
	<a href="{{$url_to_picking}}" class="btn btn-info"> <i class="icon-chevron-left"></i> {{ $button_to_picking }}</a>
	@endif
</div>
    ...
    <div class="controls">
        <div class="accordion" id="accordion2">
          <div class="accordion-group" style="background-color: #FFFFFF;">
            {{ Form::open(array('url'=>'picking/locktags', 'class'=>'form-signin', 'id'=>'form-pl-locktags', 'role'=>'form', 'method' => 'get')) }}
            <div id="collapseOne" class="accordion-body collapse in" style="padding-top: 20px;">
                    ...
                    <div class="span3">
                        <div>
                            <span class="search-po-left-pane">{{ $label_sku }}</span>
                            <span class="search-po-right-pane">
...
<div class="clear">
    ...
    <div class="div-buttons">
        ...
        <a class="btn btn-info" id="mass-unlock-pl-tag">{{ $button_unlock_tags }}</a> 
        @endif
    </div>
</div>
                ...
                @else
                    ...
                    <tr class="font-size-13 tblrow" data-id="{{ $value['lock_tag']  }}">
                        ...
                        <td class="align-center">
                            @if($value['sum_moved'] <= 0 || $value['sum_moved_qty'] <= 0)
                        ...
                        <td class="align-center">@if ( CommonHelper::valueInArray('CanUnlockPicking', $permissions) )
                            @if($value['sum_moved'] <= 0)
                            <a class="btn btn-info single-unlock-pl-tag" data-id="{{$value['lock_tag']}}" >{{ $button_unlock_tag }}</a>  
                            @else
...
<script>
    ...
    $('#submitForm').click(function() {
        $('#form-pl-locktags').submit();
    });

    $('#form-pl-locktags input').keydown(function(e) {
        if (e.keyCode == 13) {
            $('#form-pl-locktags').submit();
        }
    });
    ...
    $('.single-unlock-pl-tag').click(function() {
    ...
    $('#mass-unlock-pl-tag').click(function() {
