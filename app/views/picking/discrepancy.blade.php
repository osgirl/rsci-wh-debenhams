...
<h2><span class="label label-important" style="font-size: 15px; font-weight: normal;">Notes: Please use filter first before using the Report button.</span></h2>
	<div class="controls">
		<div class="accordion" id="accordion2">
         ...
         {{ Form::open(array('url'=>'picking/discrepansy', 'class'=>'form-signin', 'id'=>'form-purchase-order', 'role'=>'form', 'method' => 'get')) }}
...
<div class="clear">
	<div class="div-paginate">
		@if(CommonHelper::arrayHasValue($picklist_discrepancy) )
		    <h6 class="paginate">
				<span>{{ $picklist_discrepancy->appends($arrFilters)->links() }}&nbsp;</span>
			</h6>
		@else
			&nbsp;
		@endif
			      	...
			      	<div class="btn-group div-buttons">
				        ...
				        <ul class="dropdown-menu">
				          <li><a id="exportList">Export pdf</a></li>
					          <li><a id="exportlistpdf">Export excel</a></li>
				        </ul>
			      	</div>
			     </th>
			</tr>
    ...
    <div class="widget-content">
    	<div class="table-responsive">
			<table class="table table-striped table-bordered">
				...
				@else
					@foreach( $picklist_discrepancy as $doc_no )
					<tr class="font-size-13 tblrow" data-id="">
		 				...
		 				<td>{{$doc_no['from_slot_code']}}</td>
		 				<td>{{$doc_no['quantity_to_pick']}}</td>
		 				<td>{{$doc_no['firstname']. ' '.$doc_no['lastname']}}</td>
		 				<td>{{$doc_no['updated_at']}}</td>
		 				<td>{{$doc_no['moved_qty'] - $doc_no['quantity_to_pick']}}</td>
					</tr>
				 @endforeach
				 @endif
			</table>
...
$(document).ready(function() {
 ...
 /*
		...
		var filter_stock_piler = $('#filter_stock_piler').val();
