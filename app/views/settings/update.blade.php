...
<div class="widget">
    ...
    <div class="widget-content">
    	{{ Form::model($settings, array('url'=>'settings/updateData', 'class'=>'form-horizontal', 'id'=>'form-settings', 'role'=>'form', 'method' => 'post')) }}
		<div class="span2">&nbsp;</div>
		<div class="span9">
			<fieldset>
				<div class="control-group">											
					...
					<div class="controls">
						{{ Form::select('brand', $brand_options, null) }}
					</div> <!-- /controls -->				
				</div> <!-- /control-group -->
				...
				<div class="control-group">											
					...
					<div class="controls">
						@foreach ($product_identifier_options as $key => $val)
							@if (Input::old('product_identifier') == $key || $settings->product_identifier == $key)
								{{ Form::radio('product_identifier', $key, true) }}&nbsp;&nbsp;{{ $val }}&nbsp;&nbsp;
							@else
								{{ Form::radio('product_identifier', $key) }}&nbsp;&nbsp;{{ $val }}&nbsp;&nbsp;
							@endif
						@endforeach
					</div> <!-- /controls -->				
				</div> <!-- /control-group -->
				...
				<div class="control-group">											
					...
					<div class="controls">
						@foreach ($product_action_options as $key => $val)
							@if (Input::old('product_action') == $key || $settings->product_action == $key)
								{{ Form::radio('product_action', $key, true) }}&nbsp;&nbsp;{{ $val }}<br />
							@else
								{{ Form::radio('product_action', $key) }}&nbsp;&nbsp;{{ $val }}<br />
							@endif
						@endforeach
					</div> <!-- /controls -->				
				</div> <!-- /control-group -->	
