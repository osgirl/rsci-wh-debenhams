	...
	<div class="controls">
		<div class="accordion" id="accordion2">
          ...
          <div class="accordion-group" style="background-color: #FFFFFF;">
            {{ Form::open(array('url'=>'stocktransfer/stocktranferload', 'class'=>'form-signin', 'id'=>'form-shipping', 'role'=>'form', 'method' => 'get')) }}
...
<div class="clear">
 ...
 -->
 <a role="button" class="btn btn-info btn-darkblue assignPicklist" title="Assign To Picker" data-toggle="modal">Assign To Stock Piler</a>
 <a  class="btn btn-info btn-darkblue"   href="{{$url_generate_load_code}}">{{$button_gnerteload}}</a>
 
			
	<a class="btn btn-info btn-darkblue" href={{URL::to('load/loadnumbersyncstock')}}>Sync To Mobile </a>
...
<div class="widget widget-table action-table">
	<div class="widget-header"> <i class="icon-th-list"></i>
	 <h3>Subloc Transfer Load Lists</h3>
		...
		<div class="table-responsive">
			<table class="table table-striped table-bordered">
				...
				@else
					...
					<td>{{ $counter++ }}</td>
					<td> <a href="{{URL::to('stock_transfer/assignPostedTLnumberStockTransfer?loadnumber='.$lo->load_code.'&pilername='.$lo->firstname.' '.$lo->lastname.'&data_value='.$lo->data_value)}}">{{ $lo->load_code }}</a></td>
					...
					@if( $lo->is_shipped == 1 &&  $lo->assigned_to_user_id != 0 && $lo->assigned_by != 0 && $lo->data_value == 1)
							@if ( CommonHelper::valueInArray('CanAccessShipping', $permissions)) 
									...
									<a  style="width: 60px;" class="btn btn-info" href="{{URL::to('load/shipLoadstock?loadnumber='.$lo->load_code)}}">Ship</a>
					...
					<a href="{{url('load/printloadingsheetstock/' . $lo->load_code)}}" target="_blank" class="btn btn-danger">{{$col_load_sheet}}</a> 
					 <!-- <a  href="{{url('load/printpackliststock/' . $lo->load_code)}}" target="_blank" class="btn btn-danger">{{$col_mts_report}}</a> -->
					 @endif
					...
					@if( $lo->is_shipped == 0)
			 		<a style="width: 60px;" disabled class="btn btn-default" >Ship</a>
			 					@if($lo->data_value == 0)
								<a href="{{URL::to('stock_transfer/assignToTLNumber?loadnumber='.$lo->load_code.'&pilername='.$lo->firstname.' '.$lo->lastname.'&created_at='.date('M d, Y',strtotime($lo->created_at)))}}" class="btn btn-info">{{$col_A_loNum}}</a>
								@else
								<a  disabled class="btn btn-info">{{$col_A_loNum}}</a>
								@endif
					<a disabled class="btn btn-danger">{{$col_load_sheet}}</a> <!-- 
...
$('#close-add-load').click(function() {
    ...
    if (count>0) {/*
        ...
        if (count) {
            ...
            location = "assignedstockload?" + '&load_code=' + encodeURIComponent(boxes.join(','));
